...
2. **Sincronizar comandos** (mantém o menu `/` atualizado)
   - `python _core/sync-commands.py` — espelha `skills/` e `squads/*/skills/`
     para `.claude/commands/`. Idempotente.

3. **Commitar trabalho pendente**
...
4. **Consolidar no `main` (contornando o bug dos hooks de sessão — issue #1)**
